...
export async function processGenerateFiles({
  ...
  const choosePath = ({ from, fromFallback, exts = [] }: IFileMap) => {
    if (fallback) {
      return fromFallback;
    }

    if (existsSync(from)) {
      if (statSync(from).isDirectory() && readdirSync(from).length === 0) {
        return fromFallback;
      }
  ...
  const names = [];
  for (let fileItem of filesMap) {
    const { to, fromFallback } = fileItem;
    const newPath = choosePath(fileItem);
    ...
    let match;
    let newTarget = to;
    if ((match = newPath.match(extReg))) {
      newTarget = to.replace(extname(to), '') + (match[1] || match[2]);
    }
    ...
    if (newPath !== fromFallback) {
      names.push(basename(to));
    }
  }

  if (names.length > 0) {
    console.log(`${names.join(', ')} is generated by yourself template.`);
  }
}
